...
class Account{
	...
	public function login($un, $pw){
		...
		if($query->rowCount() == 1){
			$row = $query->fetch();
			if($row['banned'] == 1){
				header('Location:  https://bukkake.moe/wiki/#/banned');
	...
	public function updatePicture($pic, $un){
		if(empty($this->errorArray)){
	...
	public function updateVIP($vipStatus, $un){
		if(empty($this->errorArray)){
			$query = $this->con->prepare("UPDATE users SET vip=:vipStatus WHERE username=:un");
			$query->bindParam(":vipStatus", $vipStatus);
			$query->bindParam(":un", $un);
