...
class Video{
	...
	public function getScript(){
		return $this->sqlData["funScript"];
	}
	public function getScriptDownload(){
		$script = $this->sqlData["funScript"];
		$scriptFS = str_replace('.csv','',$script);
		if(file_exists(dirname(__FILE__).'/../../fs/'.$scriptFS.'.funscript')){
			$FunScript = $scriptFS.'.funscript';
		}else{
		if (file_exists(dirname(__FILE__).'/../../fs/'.$scriptFS)) {
			$FunScript = $scriptFS;
		}else{
			$FunScript = $script;
		}
	...
	public function getFilePath(){
		//$data = $this->sqlData["filePath"];    
		//$whatIWant = str_replace('/upload/','/upload/q_auto:low/',$data);    
	...
	public function getWatchCategory(){
		...
		$query->bindParam(":id", $cat);
		$query->execute();
		array_push($list,('<li><a href="https://bukkake.moe/category?pageno=1&cat='.$cat.'">'.$query->fetchColumn().'</a></li>'));
		}
